...
# ONLY these models may be used. Any other value (e.g. gpt-5, gpt-4.5, o1)
# will raise a ValueError at startup — preventing accidental cost explosions.
APPROVED_TRIAGE_MODELS = {"gpt-4o-mini"}
APPROVED_DEEP_REVIEW_MODELS = {"gpt-4o"}
...
class Config:
    ...
    #
    # include_news_lane: enables both primary (journal early-access / society)
    #   and secondary (trade press trial-name discovery) feed monitoring.
    # news_enrich_abstracts: follow article URLs to extract body text as the
    #   abstract for primary feed records. Adds ~1 HTTP request per record.
    #   Disable for dry runs or when rate-limiting is a concern.
    include_news_lane: bool
    news_enrich_abstracts: bool

    @staticmethod
    def from_env() -> "Config":
        triage_model = os.getenv("CARCINOS_TRIAGE_MODEL", "gpt-4o-mini")
        deep_review_model = os.getenv("CARCINOS_DEEP_REVIEW_MODEL", "gpt-4o")

        if triage_model not in APPROVED_TRIAGE_MODELS:
            raise ValueError(
                f"CARCINOS_TRIAGE_MODEL='{triage_model}' is not approved. "
                f"Allowed: {APPROVED_TRIAGE_MODELS}"
            )
        if deep_review_model not in APPROVED_DEEP_REVIEW_MODELS:
            raise ValueError(
                f"CARCINOS_DEEP_REVIEW_MODEL='{deep_review_model}' is not approved. "
                f"Allowed: {APPROVED_DEEP_REVIEW_MODELS}"
            )
        ...
        return Config(
            ...
            include_news_lane=os.getenv("CARCINOS_INCLUDE_NEWS_LANE", "true").lower() not in ("false", "0", "no"),
            news_enrich_abstracts=os.getenv("CARCINOS_NEWS_ENRICH_ABSTRACTS", "true").lower() not in ("false", "0", "no"),
        )
