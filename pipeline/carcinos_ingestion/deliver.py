...
def build_email_html(
    ...
    TIER_CARD = {
        "A": ("Practice Impacting", "#0d1f10", "1px solid #2d5c38", "#72a37a", "#152a18"),
        "B": ("Incremental",        "#0a0f1c", "1px solid #243358", "#6b8fc4", "#0d1628"),
        "C": ("Horizon",            "#0f0a18", "1px solid #3a2752", "#9b7fc4", "#160f22"),
    }
    TIER_SQUARES = {"A": "&#9632;&nbsp;&#9632;&nbsp;&#9632;", "B": "&#9632;&nbsp;&#9632;", "C": "&#9632;"}

    def _card(a, tier: str) -> str:
        label, bg, border, accent, _ = TIER_CARD[tier]
        ...
        if a.study_design:
            study_row = f"""
              <tr><td style="padding-bottom:9px;">
                <p style="margin:0;font-size:12.5px;color:rgba(255,255,255,0.65);font-family:Helvetica,Arial,sans-serif;line-height:1.6;"><strong style="color:rgba(255,255,255,0.85);">Study / regimen:</strong> {_esc(a.study_design)}</p>
              </td></tr>"""

        # Key findings (tier_rationale preferred, fall back to effect_size)
        key_findings = a.tier_rationale or a.effect_size or ""
        findings_row = ""
        if key_findings:
            findings_row = f"""
              <tr><td style="padding-bottom:9px;">
                <p style="margin:0;font-size:12.5px;color:rgba(255,255,255,0.85);font-family:Helvetica,Arial,sans-serif;line-height:1.6;font-weight:600;"><strong style="color:#ffffff;">Key findings:</strong> {_esc(key_findings)}</p>
              </td></tr>"""
        ...
        if a.one_liner:
            one_liner_row = f"""
              <tr><td style="padding-bottom:14px;">
                <p style="margin:0;font-size:12.5px;color:rgba(255,255,255,0.65);font-family:Helvetica,Arial,sans-serif;line-height:1.6;"><strong style="color:rgba(255,255,255,0.8);">So what:</strong> {_esc(a.one_liner)}</p>
              </td></tr>"""

        # Bottom row: evidence badge + read paper link
        evidence_cell = ""
        if a.evidence_strength:
            ev = _esc(a.evidence_strength.upper())
            evidence_cell = f'<td style="vertical-align:middle;"><span style="font-size:10px;font-weight:700;letter-spacing:0.5px;color:{accent};border:1px solid {accent};border-radius:4px;padding:3px 8px;font-family:Helvetica,Arial,sans-serif;">EVIDENCE {ev}</span></td>'

        source_cell = ""
        if a.source_url:
            source_cell = f'<td align="right" style="vertical-align:middle;"><a href="{_esc(a.source_url)}" style="font-size:12px;font-weight:700;color:{accent};text-decoration:none;font-family:Helvetica,Arial,sans-serif;">&#8599; Read paper</a></td>'

        bottom_row = ""
        if evidence_cell or source_cell:
            bottom_row = f"""
              <tr><td>
                <table width="100%" cellpadding="0" cellspacing="0"><tr>{evidence_cell}{source_cell}</tr></table>
              </td></tr>"""

        return f"""
        <table width="100%" cellpadding="0" cellspacing="0" style="background:{bg};border:{border};border-radius:10px;margin-bottom:10px;">
          <tr><td style="padding:16px 18px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr><td style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;color:{accent};padding-bottom:6px;font-family:Helvetica,Arial,sans-serif;">{site_label}</td></tr>
              <tr><td style="font-size:15px;font-weight:700;color:#ffffff;line-height:1.4;padding-bottom:7px;font-family:Helvetica,Arial,sans-serif;">{_esc(a.title)}</td></tr>
              <tr><td style="font-size:11px;color:rgba(255,255,255,0.35);padding-bottom:12px;font-family:Helvetica,Arial,sans-serif;">{_esc(a.journal)} &nbsp;&middot;&nbsp; {_esc(a.pub_date)}</td></tr>
              {study_row}
              {findings_row}
              {one_liner_row}
              {bottom_row}
    ...
    def _section(tier: str, items: list) -> str:
        ...
        _, _, _, accent, header_bg = TIER_CARD[tier]
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="vertical-align:middle;">
                    <span style="font-size:9px;color:rgba(255,255,255,0.25);margin-right:8px;">{squares}</span>
                    <span style="font-size:13px;font-weight:700;color:#ffffff;letter-spacing:0.3px;font-family:Helvetica,Arial,sans-serif;">{_esc(label)}</span>
                  </td>
                  <td align="right" style="vertical-align:middle;font-size:12px;color:rgba(255,255,255,0.4);font-family:Helvetica,Arial,sans-serif;">{count} finding{"s" if count != 1 else ""}</td>
                </tr>
              </table>
            </td></tr>
          </table>
