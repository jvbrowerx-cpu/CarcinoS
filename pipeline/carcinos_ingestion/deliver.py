...
def build_email_html(
    ...
    TIER_CARD = {
        "A": ("Practice Impacting", "#0d1f10", "1px solid #2d5c38", "#72a37a"),
        "B": ("Incremental",        "#0a0f1c", "1px solid #243358", "#6b8fc4"),
        "C": ("Horizon",            "#0f0a18", "1px solid #3a2752", "#9b7fc4"),
    }

    def _card(a, tier: str) -> str:
        label, bg, border, accent = TIER_CARD[tier]
        site_label = SITE_LABEL.get(a.disease_site_code or "", a.disease_site_code or "").upper()
        source_row = ""
        if a.source_url:
            source_row = f"""
            <tr><td style="padding-top:12px;">
              <a href="{_esc(a.source_url)}" style="font-size:12px;font-weight:700;color:#72a37a;text-decoration:none;">&#8599; Read paper</a>
            </td></tr>"""
        return f"""
        <table width="100%" cellpadding="0" cellspacing="0" style="background:{bg};border:{border};border-radius:10px;margin-bottom:10px;">
          <tr><td style="padding:16px 18px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr><td style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;color:rgba(255,255,255,0.4);padding-bottom:5px;">{site_label}</td></tr>
              <tr><td style="font-size:15px;font-weight:700;color:#ffffff;line-height:1.4;padding-bottom:7px;">{_esc(a.title)}</td></tr>
              <tr><td style="font-size:11px;color:rgba(255,255,255,0.35);padding-bottom:10px;">{_esc(a.journal)} &nbsp;&middot;&nbsp; {_esc(a.pub_date)} &nbsp;&middot;&nbsp; {label}</td></tr>
              <tr><td style="font-size:13.5px;color:#b0b0bc;line-height:1.6;">{a.one_liner}</td></tr>
              {source_row}
            </table>
          </td></tr>
        </table>"""
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td style="padding-left:10px;">
                <p style="margin:0;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1.6px;color:{accent};font-family:Helvetica,Arial,sans-serif;">{_esc(label)}</p>
              </td>
            </tr>
          </table>
        </td></tr>
