        ...
        if a.population:
            population_row = f"""
              <tr><td style="padding-bottom:10px;">
                <p style="margin:0;font-size:13px;color:{text_mid};font-family:Helvetica,Arial,sans-serif;line-height:1.6;"><strong style="color:{text_dark};">Relevant population:</strong> {_safe_text(a.population)}</p>
              </td></tr>"""
    ...
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td valign="middle" style="padding-left:12px;">
          ...
          <p style="margin:0;font-size:12px;color:rgba(255,255,255,0.45);font-family:Helvetica,Arial,sans-serif;">Weekly Digest &middot; {_esc(week_label)} &middot; {total} update{"s" if total != 1 else ""}{f" &middot; <strong style='color:rgba(255,255,255,0.75);'>{papers_scanned:,} papers scanned</strong>" if papers_scanned > 0 else ""}</p>
        </td>
      </tr>
    </table>
  </td></tr>
...
def build_email_text(alerts: list, scope: str, week_label: str) -> str:
    ...
    def _section(label: str, items: list) -> list[str]:
        ...
        out = [f"── {label.upper()} ──", ""]
        for a in items:
            out.append(f"• {a.title or '(no title)'}")
            if a.journal:
                out.append(f"  Journal: {a.journal}")
            if a.effect_size:
                out.append(f"  Key finding: {a.effect_size}")
            if a.one_liner:
                out.append(f"  So what: {a.one_liner}")
            if a.source_url:
                out.append(f"  Link: {a.source_url}")
            out.append("")
        return out

    lines += _section("Practice Impacting", pi)
    lines += _section("Clinically Relevant", inc)
    lines += _section("Horizon", hor)

    lines += [
        "-" * 50,
        "View the full digest: https://carcino-s.com/this-week",
        "Manage preferences / Unsubscribe: https://carcino-s.com/settings",
        "",
        "CarcinoS · Salient Oncology Intelligence · carcino-s.com",
    ]
    return "\n".join(lines)
...
def send_email(
    ...
    try:
        ...
        params = {
            ...
            "subject": subject,
            "html": html_body,
            # Plain-text fallback — required for deliverability at institutional
            # mail filters (Proofpoint, Mimecast). HTML-only emails score poorly.
            "text": text_body or subject,
            # List-Unsubscribe headers — required for bulk/digest email.
            # Institutional spam filters (especially Proofpoint) look for these
            # in any recurring digest. RFC 8058 one-click variant is included
            # alongside the mailto: fallback for maximum compatibility.
            "headers": {
                "List-Unsubscribe": f"<{unsubscribe_url}>, <mailto:unsubscribe@carcino-s.com?subject=unsubscribe>",
...
def run_delivery(config: Config, days: int = 7, dry_run: bool = False) -> dict:
    ...
    papers_scanned = 0
    try:
        run_rows = sb.table("pipeline_runs") \
            .select("stats_json") \
            .order("run_date", desc=True) \
            .limit(1) \
            .execute()
        if run_rows.data:
            papers_scanned = run_rows.data[0].get("stats_json", {}).get("papers_fetched", 0) or 0
    except Exception:
        pass  # non-critical
    ...
    for sub in subscribers:
        ...
        # the article was included in a prior successful send.
        try:
            delivered_rows = sb.table("digest_deliveries") \
                .select("alert_id") \
                .eq("user_id", user_id) \
                .execute()
            already_sent = {r["alert_id"] for r in (delivered_rows.data or [])}
        except Exception as e:
            # Non-fatal: if the table doesn't exist yet, skip dedup rather than
            # blocking delivery.  Log loudly so the operator notices.
            print(f"    [WARN] digest_deliveries query failed (run migration 09?): {e}", file=sys.stderr)
            already_sent = set()

        before_dedup = len(alerts)
        alerts = [a for a in alerts if a.id not in already_sent]
        deduped = before_dedup - len(alerts)
        if deduped:
            print(f"    Deduped {deduped} alert(s) already sent in a prior digest")

        if not alerts:
            print(f"    No new alerts this week — skipping")
            stats["skipped"] += 1
            continue

        # Only send if there is at least one Practice Impacting (tier A) alert.
        # Incremental/Horizon-only weeks don't warrant an email.
        pi_alerts = [a for a in alerts if a.tier == "A"]
        if not pi_alerts:
            print(f"    No Practice Impacting alerts this week — skipping ({len(alerts)} lower-tier alert(s) held)")
            stats["skipped"] += 1
            continue
        ...
        if delivery in ("push", "both") and token:
            ...
            push_ok = send_push(token, messages, dry_run=dry_run)
            if push_ok:
                stats["push_sent"] += 1

        # Record deliveries so these alerts are never re-sent to this user.
        # Only record on a successful send (email or push) to avoid marking
        # articles as "delivered" when the send actually failed.
        # In dry-run mode, send_email/send_push return True — we intentionally
        # skip recording so dry runs don't poison the dedup table.
        if (email_ok or push_ok) and not dry_run and already_sent is not None:
            rows_to_insert = [
                {"user_id": user_id, "alert_id": a.id}
                for a in alerts
            ]
            try:
                sb.table("digest_deliveries") \
                    .upsert(rows_to_insert, on_conflict="user_id,alert_id") \
                    .execute()
                print(f"    Recorded {len(rows_to_insert)} delivery row(s)")
            except Exception as e:
                # Non-fatal: article was delivered; dedup will re-try next run.
                print(f"    [WARN] Failed to record deliveries: {e}", file=sys.stderr)
