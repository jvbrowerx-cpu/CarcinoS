...
def build_email_html(
    ...
    def _section(tier: str, items: list) -> str:
        ...
        for a in items:
            ...
            card_bg = "rgba(106,184,122,0.05)" if tier == "A" else "rgba(255,255,255,0.03)"
            card_bd = "rgba(106,184,122,0.2)" if tier == "A" else "rgba(255,255,255,0.09)"
            cards += f"""
            <div class="card {tier_class}" style="background:{card_bg};border:1px solid {card_bd};border-radius:10px;padding:16px 18px;margin-bottom:10px;">
              <div class="card-site" style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;color:rgba(255,255,255,0.4);margin-bottom:5px;">{_esc(site_label)}</div>
              <div class="card-title" style="font-size:15px;font-weight:700;color:#ffffff;line-height:1.4;margin-bottom:7px;">{_esc(a.title)}</div>
              <div class="card-meta" style="font-size:11px;color:rgba(255,255,255,0.35);margin-bottom:9px;">
                {_esc(a.journal)}<span style="padding:0 5px;">·</span>{_esc(a.pub_date)}<span style="padding:0 5px;">·</span>{signal}
              </div>
              <div class="card-body" style="font-size:13.5px;color:#b0b0bc;line-height:1.55;">{a.one_liner}</div>
              {source_link}
            </div>"""
        return f'<div class="section-label" style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;color:rgba(255,255,255,0.4);margin:28px 0 12px;">{_esc(label)}</div>{cards}'
...
<div class="wrapper" style="background-color:#060609;max-width:640px;margin:0 auto;padding:32px 24px 60px;">
  <div class="header">
