...
def build_email_html(
    ...
    def _card(a, tier: str) -> str:
        ...
        study_display = _esc(a.study_design) if a.study_design else '<em style="color:{muted};">See full paper</em>'.format(muted=text_muted)
        study_row = f"""
              <tr><td style="padding-bottom:10px;">
                <p style="margin:0;font-size:13px;color:{text_mid};font-family:Helvetica,Arial,sans-serif;line-height:1.6;"><strong style="color:{text_dark};">Study / regimen</strong> (high level): {study_display}</p>
              </td></tr>"""

        # Key findings — effect_size is the human-readable finding sentence from the LLM
        key_findings_text = a.effect_size or ""
        key_display = _esc(key_findings_text) if key_findings_text else '<em style="color:{muted};">See full paper</em>'.format(muted=text_muted)
        findings_row = f"""
              <tr><td style="padding-bottom:10px;">
        ...
        one_liner_display = _esc(a.one_liner) if a.one_liner else '<em style="color:{muted};">See full paper</em>'.format(muted=text_muted)
        one_liner_row = f"""
