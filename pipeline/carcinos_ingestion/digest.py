...
SITE_LABEL = {
    ...
    "sarcoma": "Sarcoma", "pediatric": "Pediatric", "benign": "Benign",
}
...
class FlatAlert:
    ...
    tags: list[str]
    source_url: str = ""
    # Audience routing — which subscriber groups should receive this alert.
    # Defaults to both so that records fetched without this field (e.g. from
    # fetch_published) behave exactly as before.
    audience: list = None  # type: ignore[assignment]
    # Conference source — set when this alert was manually added as a conference
    # abstract (e.g. "ASTRO 2026"). Empty string for regular pipeline alerts.
    conference_source: str = ""

    def __post_init__(self):
        if self.audience is None:
            self.audience = ['broad_oncology', 'radiation_oncology']
...
def _guideline_study_design(s: dict) -> str:
    """
    For GUIDELINE alerts, synthesise a study_design string from
    recommendation_changes since regimen_description is absent.
    Returns the first change's previous→new pair as a readable sentence.
    """
    body = s.get("guideline_body") or ""
    version = s.get("guideline_version_or_year") or ""
    disease = s.get("disease_area") or ""
    if body or disease:
        header = f"{body} {version}".strip()
        return f"{header} guideline update — {disease}".strip(" —")
    return ""


def _guideline_effect_size(s: dict) -> str:
    """
    For GUIDELINE alerts, synthesise a key finding from the first
    recommendation_change (new recommendation text).
    """
    changes = s.get("recommendation_changes") or []
    if changes:
        first = changes[0]
        new_rec = (first.get("new") or "").strip()
        strength = first.get("strength") or ""
        if strength:
            return f"{new_rec} [{strength}]" if new_rec else ""
        return new_rec
    return ""
...
def _flatten_alert(row: dict[str, Any]) -> FlatAlert:
    ...
    # Detect GUIDELINE schema (has recommendation_changes, no regimen_description)
    is_guideline = bool(s.get("recommendation_changes"))
    if is_guideline:
        study_design_text = _guideline_study_design(s)
        effect_size_text = _guideline_effect_size(s)
    else:
        study_design_text = s.get("regimen_description") or ""
        kr = s.get("key_results") or {}
        _finding_parts = [
            p for p in [
                kr.get("effect_size"),
                kr.get("p_value_or_ci"),
                kr.get("absolute_difference"),
            ]
            if p and p != "not_reported"
        ]
        effect_size_text = " · ".join(_finding_parts) or s.get("primary_endpoint") or ""
    ...
    return FlatAlert(
        ...
        audience=row.get("audience") or ['broad_oncology', 'radiation_oncology'],
        conference_source=s.get("conference_source") or "",
    )
...
def fetch_for_user(
    ...
    if as_of is None:
        as_of = date.today()
    # Anchor to Monday of the current week so the digest never reaches into
    # the prior week, regardless of which day the delivery script runs.
    monday = _monday_of_week(as_of)
    since = datetime.combine(monday, datetime.min.time(), tzinfo=timezone.utc)
    ...
    def _flat(row: dict) -> FlatAlert:
        ...
        src0 = sources[0] if sources else {}
        trial = s.get("trial") or {}
        # Build key findings the same way the this-week page does:
        # combine effect_size + p_value_or_ci + absolute_difference joined with " · "
        # key_results may be at the flat top-level path (new schema)
        # or nested under results.key_results (legacy/manually-inserted records)
        kr = s.get("key_results") or (s.get("results") or {}).get("key_results") or {}
        if s.get("recommendation_changes"):
            study_design_text = _guideline_study_design(s)
            effect_size_text = _guideline_effect_size(s)
        else:
            study_design_text = s.get("regimen_description") or ""
        ...
        return FlatAlert(
            ...
            pub_date=src0.get("publication_date") or s.get("publication_date") or "",
            evidence_strength=s.get("evidence_strength") or "",
            tier_rationale=s.get("tier_rationale") or "",
            study_design=study_design_text,
            effect_size=effect_size_text,
            population=s.get("population") or "",
            tags=list(s.get("who_should_care") or []),
