...
def _flatten_alert(row: dict[str, Any]) -> FlatAlert:
    ...
    if is_guideline:
        ...
        effect_size_text = (s.get("key_results") or {}).get("effect_size") or ""
...
def fetch_for_user(
    ...
    def _flat(row: dict) -> FlatAlert:
        ...
        return FlatAlert(
            ...
            tier_rationale=s.get("tier_rationale") or "",  # admin audit text — not for subscribers
            study_design=(
                _guideline_study_design(s) if s.get("recommendation_changes")
                else s.get("regimen_description") or ""
            ),
            effect_size=(
                _guideline_effect_size(s) if s.get("recommendation_changes")
                else (s.get("key_results") or {}).get("effect_size") or ""
            ),
            tags=list(s.get("who_should_care") or []),
            source_url=_source_url(src0, trial),
        )
