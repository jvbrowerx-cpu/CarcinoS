...
def _flatten_alert(row: dict[str, Any]) -> FlatAlert:
    ...
    return FlatAlert(
        ...
        journal=src0.get("journal") or "",
        pub_date=src0.get("publication_date") or "",
        evidence_strength=cls.get("evidence_strength") or "",
        tier_rationale=str(res.get("tier_rationale") or cls.get("tier_rationale") or ""),
        study_design=trial.get("study_design") or "",
        effect_size=res.get("effect_size") or "",
...
def fetch_for_user(
    ...
    def _flat(row: dict) -> FlatAlert:
        ...
        return FlatAlert(
            ...
            evidence_strength=(cls.get("evidence_strength") or s.get("evidence_strength") or ""),
            tier_rationale=str(res.get("tier_rationale") or s.get("tier_rationale") or ""),
            study_design=trial.get("study_design") or s.get("study_design") or "",
            effect_size=(res.get("key_results") or {}).get("effect_size") or "",
            tags=list(cls.get("who_should_care") or s.get("who_should_care") or []),
            source_url=_source_url(src0, trial),
        )
