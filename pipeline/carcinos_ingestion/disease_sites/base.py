...
# Spec §1.2 — study-type expansion.
# INTENTIONALLY NARROW: we want Phase III RCTs, guidelines, and FDA approvals.
# We do NOT search for meta-analyses, systematic reviews, cohorts, registries,
# epidemiology studies, or observational data — those are noise by default and
# will never carry a qualifying signal anyway.
STUDY_TYPE_TERMS: Sequence[str] = (
    ...
    "clinical trial",
    "guideline", "consensus",
    "FDA approved", "FDA approval",
    "practice-changing", "standard of care",
)
...
TIER1_JOURNAL_WHITELIST: Sequence[str] = (
    # Full names (used in _journal_block() PubMed queries)
    ...
    "JAMA",
    "JAMA Oncology",
    # PubMed abbreviations — these are what PubMed stores in the journal field
    # of returned records. Without these, matches_journal_whitelist() misses
    # NEJM and Lancet Oncology papers even though the search query finds them.
    "N Engl J Med",       # NEJM
    "Lancet Oncol",       # Lancet Oncology
    "J Clin Oncol",       # Journal of Clinical Oncology (safety net)
    "JAMA Oncol",         # JAMA Oncology (safety net)
)
...
# Quick-match term lists used to route non-PubMed records (e.g. FDA approvals)
# to the correct disease site without running a full PubMed query. Terms are
# matched case-insensitively as substrings against the record's title + abstract.
SITE_QUICK_TERMS: dict[str, list[str]] = {
    "gynecologic": [
        "ovarian", "cervical", "endometrial", "uterine", "vulvar",
        "fallopian tube", "gynecologic", "gynecological",
    ],
    "thoracic": [
        "lung", "nsclc", "sclc", "non-small-cell lung", "non-small cell lung",
        "small cell lung", "mesothelioma", "thymoma",
    ],
    "breast": ["breast"],
    "head_neck": [
        "head and neck", "laryngeal", "pharyngeal", "oropharyngeal",
        "nasopharyngeal", "oral cavity", "thyroid", "salivary gland",
    ],
    "cns": [
        "glioblastoma", "glioma", "brain tumor", "brain tumour", "gbm",
        "astrocytoma", "medulloblastoma", "meningioma", "central nervous system tumor",
    ],
    "sarcoma": [
        "sarcoma", "gastrointestinal stromal", "gist", "osteosarcoma",
        "ewing sarcoma", "rhabdomyosarcoma", "liposarcoma",
    ],
    "gu": [
        "prostate", "bladder", "renal cell", "kidney cancer", "urothelial",
        "testicular", "penile",
    ],
    "hematologic": [
        "leukemia", "lymphoma", "myeloma", "myelodysplastic", "myeloid",
        "lymphoblastic", "cll", "aml", "dlbcl", "hodgkin", "multiple myeloma",
        "myelofibrosis", "polycythemia vera", "essential thrombocythemia",
    ],
    "cutaneous": [
        "melanoma", "basal cell carcinoma", "merkel cell", "cutaneous squamous",
        "squamous cell carcinoma of the skin",
    ],
    "gastrointestinal": [
        "colorectal", "colon cancer", "rectal cancer", "gastric", "stomach cancer",
        "pancreatic", "esophageal", "hepatocellular", "liver cancer", "biliary",
        "cholangiocarcinoma", "gastroesophageal junction", "hcc", "crc",
    ],
    "pediatric": [
        "pediatric", "paediatric", "childhood cancer", "childhood malignancy",
        "neuroblastoma", "medulloblastoma", "Wilms tumor", "nephroblastoma",
        "rhabdomyosarcoma", "hepatoblastoma", "retinoblastoma",
        "DIPG", "diffuse intrinsic pontine glioma", "diffuse midline glioma",
        "atypical teratoid", "ATRT",
        "Langerhans cell histiocytosis",
        "juvenile myelomonocytic leukemia", "JMML",
        "adolescent and young adult", "AYA",
        "Children's Oncology Group", "COG", "SIOP",
    ],
}
...
class DiseaseSiteConfig:
    ...
    watched_trials: Sequence[str] = field(default_factory=tuple)
    # Curated list of major ongoing or recently-reporting trial names/acronyms
    # and NCT numbers for this disease site. Any paper whose title or abstract
    # contains one of these strings (word-boundary match) automatically receives
    # QS_WATCHED_TRIAL — the highest non-FDA/GUIDELINE qualifying signal.
    # Use short trial acronyms (e.g. "LAURA", "PATHOS") or NCT IDs.
