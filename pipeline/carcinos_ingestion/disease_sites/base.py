...
SHARED_JOURNAL_WHITELIST: Sequence[str] = (
    ...
    # AACR flagship → covers AACR Annual Meeting supplements
    "Cancer Discovery",
    "Cancer Research",
    # Immunotherapy / IO
...
def _date_block(start: date, end: date) -> str:
    """PubMed date publication range filter."""
    fmt = "%Y/%m/%d"
    return f'("{start.strftime(fmt)}"[Date - Publication] : "{end.strftime(fmt)}"[Date - Publication])'
