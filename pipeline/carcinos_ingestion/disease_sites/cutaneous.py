...
CONFIG = DiseaseSiteConfig(
    ...
    watched_trials=(
        ...
        "KEYNOTE-054",        # Pembrolizumab adjuvant stage III melanoma (long-term OS)
        "CheckMate 238",      # Nivolumab vs ipilimumab adjuvant melanoma (long-term)
        "COMBI-AD",           # Dabrafenib + trametinib adjuvant BRAF+ melanoma (OS)
        # Melanoma — advanced
        "RELATIVITY-047",     # Relatlimab + nivolumab vs nivolumab 1L advanced melanoma
        "CheckMate 067",      # Nivolumab + ipilimumab melanoma (10-year OS updates)
        "COLUMBUS",           # Encorafenib + binimetinib vs vemurafenib BRAF+ melanoma
        "DREAMseq",           # BRAF+MEKi vs IO sequencing strategy melanoma
        "SECOMBIT",           # Encorafenib + binimetinib vs IO sequencing melanoma
        "IMspire150",         # Atezolizumab + cobimetinib + vemurafenib BRAF+ melanoma
        "PIVOT IO 001",       # Fianlimab + cemiplimab 1L advanced melanoma
        "KEYNOTE-942",        # mRNA-4157 + pembrolizumab adjuvant high-risk melanoma
        "RAMPART",            # Nivolumab ± ipilimumab adjuvant resected stage III/IV melanoma
        # Uveal melanoma
        "IMCgp100-202",       # Tebentafusp vs investigator choice uveal melanoma
        # Merkel cell
        "KEYNOTE-017",        # Pembrolizumab MCC (long-term updates)
        "JAVELIN Merkel 200", # Avelumab MCC (long-term updates)
        # cSCC / BCC
        "C-POST",             # Cemiplimab adjuvant high-risk cSCC post-surgery
        "KEYNOTE-630",        # Pembrolizumab adjuvant high-risk cSCC
        "EMPOWER-CSCC-1",     # Cemiplimab locally advanced/metastatic cSCC
        "KEYNOTE-629",        # Pembrolizumab recurrent/metastatic cSCC
    ),
)
