...
CONFIG = DiseaseSiteConfig(
    code="hematologic",
    name="Hematologic",
    # Hematologic malignancies rarely use "cancer/tumor/carcinoma" in abstracts.
    # Override tumor_terms so that core_block() matches myeloma, lymphoma, and
    # leukemia papers that only use disease-specific terminology.
    tumor_terms=(
        "cancer", "carcinoma", "malignancy", "neoplasm", "tumor", "tumour",
        "myeloma", "lymphoma", "leukemia", "leukaemia", "lymphoproliferative",
    ),
    free_text_core=(
        "lymphoma", "Hodgkin lymphoma", "Hodgkin's", "non-Hodgkin",
        "nodular lymphocyte-predominant Hodgkin", "NLPHL",
    ...
    watched_trials=(
        ...
        # Multiple myeloma
        "ENDURANCE",          # E1A11 — continuous vs fixed-duration maintenance MM
        "CARTITUDE-1",        # Ciltacabtagene autoleucel RRMM (foundational CAR-T)
        "MAIA",               # Daratumumab + Rd vs Rd transplant-ineligible ND myeloma
