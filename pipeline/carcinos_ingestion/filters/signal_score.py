...
_GUIDELINE_BODY_RE = re.compile(
    r"\b(nccn|asco|esmo|astro|ash|esgo|eortc|aua|nci|who"
    r"|aad|american academy of dermatology"   # cutaneous guidelines
    r"|aua|sua|eau"                            # urology societies
    r"|snmmi|abnm"                             # nuclear medicine
    r")\b",
    re.IGNORECASE,
)
...
_TOP_TIER_JOURNALS: frozenset[str] = frozenset({
    ...
    # Dermatology — added for cutaneous guideline coverage
    "jama dermatology",
...
# Extended journal whitelist for filtering WEAK signals (QS_META_ANALYSIS and
# QS_PHASE_II_RANDOMIZED).  These signals have the broadest admission criteria
# and without a journal gate they let through papers from minor or non-oncology
# journals.  Any paper whose *only* qualifying signal is one of these two must
# be from a journal in this set; otherwise it is demoted to QS_NONE.
#
# This set is broader than _TOP_TIER_JOURNALS — it includes disease-site
# flagship journals and well-regarded subspecialty oncology titles.
_ACCEPTABLE_JOURNALS_FOR_WEAK_SIGNALS: frozenset[str] = frozenset({
    # Top-tier (same as _TOP_TIER_JOURNALS)
    "new england journal of medicine", "nejm", "nejm evidence",
    ...
    "journal of hematology and oncology",
    "clinical cancer research",
    # Subspecialty oncology — broadly used, peer-reviewed
    ...
    # Other well-regarded oncology titles
...
def score_candidate(
    ...
    # ── Journal quality gate (applies to weak signals only) ──────────────────
    #
    # QS_META_ANALYSIS and QS_PHASE_II_RANDOMIZED have the broadest admission
    # criteria — they fire on any paper mentioning "systematic review" or
    # "randomized phase II" regardless of journal quality.  Without a journal
    # gate, papers from minor or non-oncology journals flood the queue.
    #
    # Rule: if the ONLY qualifying signal is QS_META_ANALYSIS or
    # QS_PHASE_II_RANDOMIZED, the journal must be in the extended acceptable
    # list.  Strong signals (Phase III, FDA, Guideline, etc.) are unaffected.
    _WEAK_SIGNALS = frozenset({QS_META_ANALYSIS, QS_PHASE_II_RANDOMIZED})
    if qs in _WEAK_SIGNALS:
        journal_lower = (cand.journal or "").lower().strip()
        journal_ok = False
        if journal_lower:
            if journal_lower in _ACCEPTABLE_JOURNALS_FOR_WEAK_SIGNALS:
                journal_ok = True
            else:
                # Substring match handles abbreviated/variant journal names
                for accepted in _ACCEPTABLE_JOURNALS_FOR_WEAK_SIGNALS:
                    if len(accepted) >= 10 and (accepted in journal_lower or journal_lower in accepted):
                        journal_ok = True
                        break
        if not journal_ok:
            return SignalScore(
                ...
                score=0,
                hard_excluded=False,
                rejection_reason="non_whitelist_journal",
            )
