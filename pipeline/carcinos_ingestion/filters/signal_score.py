...
QUALIFYING_SIGNALS = frozenset({
    ...
    QS_META_ANALYSIS, QS_TOP_JOURNAL, QS_FORCE_KEEP,
})
...
_HARD_EXCLUDE_RE = re.compile(
    ...
    # Surgical approach comparisons (laparoscopic vs open) — structural
    # feasibility / technique comparisons are never practice-changing for
    # the oncology pipeline; they are surgical skill, not treatment strategy.
    r"|laparoscopic.{0,40}(versus|vs\.?|compared (to|with)).{0,40}(laparotom|open.{0,10}(surgery|approach|resection|procedure))"
    r"|(laparotom|open.{0,10}(surgery|approach|resection)).{0,40}(versus|vs\.?|compared (to|with)).{0,40}laparoscopic"
    r"|robotic.{0,40}(versus|vs\.?|compared (to|with)).{0,40}(laparoscopic|open|laparotom)"
    r")\b",
    re.IGNORECASE,
)
...
_HARD_EXCLUDE_TITLE_ONLY_RE = re.compile(
    ...
    # Retrospective series in titles: "retrospective study/analysis/cohort"
    # without any trial language. Caught title-only to avoid false positives
    # in discussion sections that mention retrospective context.
    r"|retrospective.{0,20}(multicenter|multi.center|single.center|single.institution|population.based).{0,20}(study|analysis|cohort|review|series)"
    r"|(retrospective series|retrospective chart review|retrospective database analysis)"
    r")\b",
    re.IGNORECASE,
)
...
def _is_phase3_rct(text: str) -> bool:
    ...
    # Reject if the abstract is primarily describing a retrospective design.
    if _PHASE3_RETROSPECTIVE_RE.search(text):
        return False
    return True
...
def _additive_score(
    ...
    _QS_BASE = {
        ...
        QS_NEGATIVE_TRIAL:          18,   # negative RCT — practice-defining "what NOT to do"
