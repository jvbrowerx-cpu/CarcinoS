...
class CanonicalCandidate:
    ...
    # Conference abstract provenance (set when sourced from the conference lane)
    is_conference_abstract: bool = False
    conference_source: Optional[str] = None  # e.g. "ASCO", "ESMO", "ASTRO", "WEB_SEARCH"

    # PMC ID — present when article is in PubMed Central OA; used for full-text
    # enrichment of living-guideline records whose abstracts lack recommendation detail.
    pmc_id: Optional[str] = None

    # For the persistence layer
    raw_xml_hash: str = ""
    source_url: str = ""  # canonical URL for this record (real URL for web search; PubMed URL for others)

    # Set when the post-Pass-2 site correction overrides the retrieval-assigned site
    site_auto_corrected: bool = False
    site_auto_corrected_from: Optional[str] = None
...
def to_canonical(
    ...
    return CanonicalCandidate(
        ...
        is_conference_abstract=bool(getattr(record, "conference_source", None)),
        conference_source=getattr(record, "conference_source", None),
        pmc_id=record.pmc_id or None,
