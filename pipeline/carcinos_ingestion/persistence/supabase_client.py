...
class SupabaseClient:
    ...
    def upsert_source_and_trial(
        ...
        # ---- source ----
        # Use the real URL from the candidate (web search records carry their actual article URL;
        # PubMed records fall back to the standard PubMed URL).
        source_url = getattr(candidate, "source_url", None) or f"https://pubmed.ncbi.nlm.nih.gov/{candidate.pmid}/"
        # Determine source type — web search lane sets conference_source="WEB_SEARCH"
        source_type = (
            "web_search"
            if getattr(candidate, "conference_source", None) == "WEB_SEARCH"
            else _infer_source_type(candidate.publication_types)
        )
        source_payload = {
            "trial_id": trial_id,
            "type": source_type,
    ...
    def insert_alert(
        ...
        # Extract qualifying signal from Pass 1 one_line_rationale field
        _qs_match = _re.match(r"qualifying_signal=([^;]+)", pass1.parsed.get("one_line_rationale", ""))
        qualifying_signal = _qs_match.group(1).strip() if _qs_match else None

        # Real article URL (web search records carry their own URL; PubMed records use standard URL)
        alert_source_url = getattr(candidate, "source_url", None) or f"https://pubmed.ncbi.nlm.nih.gov/{candidate.pmid}/"
        ...
        summary_json = {
            ...
            # Denormalized site code — lets the frontend filter by site without
            # joining disease_sites or relying on PostgREST FK embedding.
            "disease_site_code": candidate.cancer_site_code,
            ...
            # Fields added in Tasks 26–29
            "trial_name": pass2.parsed.get("trial_name"),                          # Task 27 — trial dedup key
            "why_it_surfaced": pass2.parsed.get("why_it_surfaced"),                # Task 26 — editorial transparency
            "radiation_oncology_relevance": pass2.parsed.get("radiation_oncology_relevance"),
            "confidence": pass2.parsed.get("confidence"),
            "reject_recommendation": pass2.parsed.get("reject_recommendation"),
            "qualifying_signal": qualifying_signal,                                 # Task 26 — which Gate 2 rule fired
            "conference_source": candidate.conference_source,                      # Task 29 — lane provenance
            "is_conference_abstract": candidate.is_conference_abstract,
            # Site correction provenance — set when post-Pass-2 routing overrode retrieval assignment
            "site_auto_corrected": getattr(candidate, "site_auto_corrected", False),
            "site_auto_corrected_from": getattr(candidate, "site_auto_corrected_from", None),
            ...
            "sources": [{
                ...
                "publication_date": candidate.publication_date,
                "text_hash": candidate.text_hash,
                "conference_source": candidate.conference_source,
        ...
        # Flatten radiation_oncology_relevance so SQL can filter on it directly
        # (it is also stored in summary_json for backward compat / display)
        rad_relevance = pass2.parsed.get("radiation_oncology_relevance")  # 'direct' | 'indirect' | 'none'
        ...
        payload = {
            ...
            "notify": pass2.notify,
            # Flat column added in migration 04 — enables scope-based delivery filtering
            "radiation_oncology_relevance": rad_relevance,
        }
