...
def run_site(
    ...
    *,
    ...
    # 90-day publication date floor — [Date - Entrez] can surface late-indexed
    # older papers; this keeps the feed genuinely fresh.
    from datetime import timedelta
    ...
    for r, pubtype_decision in pubtyped:
        ...
        # FDA records are always force-kept — any FDA oncology approval is
        # high-signal by definition; the RSS filter already qualified it.
        if r.conference_source == "FDA":
            force_keep_reason = "fda_approval"

        # Top-tier journal whitelist force-keep
        elif r.matches_journal_whitelist(whitelist):
            if rel.keep:
