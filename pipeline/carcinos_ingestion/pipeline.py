    ...
    if no_signal:
        ...
        if gated:
            ...
            for cand, sig in gated:
                try:
                    p1 = run_pass1(openai_client, cand, model=config.triage_model)
                    p1 = enforce_pass1_keep_rules(p1, cand, sig)

                    p1_score = p1.relevance_score

                    if p1_score >= config.qs_none_promote_threshold and p1.keep:
                        # Promote to Pass 2
                    ...
                    elif p1_score >= config.qs_none_quarantine_threshold and p1.keep:
