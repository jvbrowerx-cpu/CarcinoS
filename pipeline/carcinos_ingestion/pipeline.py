...
from .triage.pass2 import run_pass2, run_pass2_guideline, verify_evidence_quotes, Pass2Result
...
class RunStats:
    ...
    # Populated during retrieval and alert creation so the admin coverage panel
    # can show "JCO: 34 fetched → 6 kept" without a separate query.
    journals_fetched: dict[str, int] = field(default_factory=dict)   # journal → raw fetch count
    journals_kept: dict[str, int] = field(default_factory=dict)       # journal → published alert count
...
def run_site(
    ...
    *,
    ...
    # Count raw fetches per journal (normalised: strip "The ", lowercase for grouping)
    for r in records:
        j = (getattr(r, "journal", None) or "Unknown").strip()
        stats.journals_fetched[j] = stats.journals_fetched.get(j, 0) + 1
    ...
    # papers (PubMed indexing lag is typically 1-4 weeks). We allow a 14-day
    # grace period: newly-indexed papers up to 2 weeks old are still clinically
    # current; anything older is stale for a weekly digest.
    from datetime import timedelta
    ...
    def _pub_date_ok(r: PubMedRecord) -> bool:
        ...
        try:
            ...
            pub = date(y, m, d)
            # Lower bound: reject stale papers (>14 days old)
            # Upper bound: reject future-dated papers (pub_date > run end date)
            # Future dates occur when PubMed indexes an ahead-of-print article
            # before its official publication date; surfacing them is misleading.
            return cutoff <= pub <= end
        except Exception:
            return True
    ...
    if dropped_old:
        log.info("[%s] %d papers removed (outside window %s – %s)",
                 site.code, dropped_old, cutoff.isoformat(), end.isoformat())
    ...
    for r, pubtype_decision in pubtyped:
        ...
        elif r.matches_journal_whitelist(list(TIER1_JOURNAL_WHITELIST)):
            # Tier-1 journal papers (NEJM, Lancet, Lancet Oncology, JCO, JAMA,
            # JAMA Oncology) bypass Gate 2 signal scoring entirely and go
            # straight to Pass 2. The journal's peer-review IS the qualifying
            # filter. Without this, Phase II single-arm trials and other
            # clinically important papers from these journals are silently
            # dropped because they don't earn a qualifying signal code.
            # Pass 2 (gpt-4o) decides NOISE vs. A/B/C.
            force_keep_reason = "tier1_journal"

        # Journal whitelist bypass — papers retrieved via the curated journal
        # lanes already passed a cancer-site keyword filter as part of the
        # PubMed query. Running F3 relevance scoring on top is double-gating
        # and silently drops legitimate papers that score 25–64.
        # Non-Tier-1 whitelist papers still go through Gate 2 (qualifying signal).
    ...
    def _do_pass2(cand: CanonicalCandidate, sig: SignalScore, model: str, is_no_signal: bool):
        ...
        # Route GUIDELINE signals to a dedicated extraction path that
        # extracts recommendation changes instead of trial HR/ORR/p-values.
        if sig.qualifying_signal == "GUIDELINE":
            p2 = run_pass2_guideline(
    ...
    with ThreadPoolExecutor(max_workers=_LLM_WORKERS) as pool:
        ...
        for fut in as_completed(futures2):
            try:
                ...
                # Post-Pass-2 site correction.
                # The LLM extracts cancer_site_subtype for every paper (required
                # field). If it clearly identifies a different disease site than
                # what retrieval assigned, auto-correct before persisting and flag
                # the card so it's visible in the admin portal.
                # This is a second layer on top of the retrieval-time routing fix —
                # it catches any edge case that slipped through.
                _p2_subtype = (p2.parsed.get("cancer_site_subtype") or "").lower()
                if _p2_subtype:
                    _cur_quick = [t.lower() for t in SITE_QUICK_TERMS.get(cand.cancer_site_code, [])]
                    if _cur_quick and not any(t in _p2_subtype for t in _cur_quick):
                        _suggested = next(
                            (code for code, terms in SITE_QUICK_TERMS.items()
                             if code != cand.cancer_site_code
                             and any(t.lower() in _p2_subtype for t in terms)),
                            None,
                        )
                        if _suggested:
                            log.warning(
                                "[%s] AUTO-CORRECTING site: PMID %s '%s' → '%s' "
                                "(cancer_site_subtype='%s')",
                                site.code, cand.pmid, cand.cancer_site_code,
                                _suggested, _p2_subtype[:80],
                            )
                            cand.site_auto_corrected = True
                            cand.site_auto_corrected_from = cand.cancer_site_code
                            cand.cancer_site_code = _suggested
                            cand.cancer_site_name = ALL_SITES[_suggested].name
                ...
                if p2.final_tier_code in ("A", "B", "C"):
                    ...
                    # Track per-journal kept count (protected by _stats_lock above)
                    j = (cand.journal or "Unknown").strip()
                    with _stats_lock:
                        stats.journals_kept[j] = stats.journals_kept.get(j, 0) + 1
            except Exception as e:
                cand, *_ = futures2[fut]
...
def _retrieve(
    ...
    *,
    ...
    # Lane 2a: Tier-1 guaranteed (NEJM / Lancet / Lancet Oncol / JCO / JAMA) — never capped
    # Uses a 14-day lookback (vs. 7-day for other lanes) to handle PubMed indexing
    # lag: Lancet Oncology and NEJM papers can take 5–10 days to receive an Entrez
    # date after online publication, causing them to be missed by a 7-day window.
    # Also uses study_type_block() instead of core_block() so papers are not
    # filtered by disease-site keywords — a thyroid cancer trial in Lancet Oncology
    # should not need to match the head/neck site's keyword list to be retrieved.
    from datetime import timedelta as _td
    tier1_start = end - _td(days=14)
    tier1_q = " AND ".join([
        site.study_type_block(),
        _journal_block(TIER1_JOURNAL_WHITELIST),
        _date_block(tier1_start, end),
    ])
    tier1_pmids = pubmed.esearch(tier1_q, retmax=max_pmids)
    log.info("[%s] esearch (tier-1 guaranteed, 14-day): %d PMIDs", site.code, len(tier1_pmids))
    ...
    # Tier-1 cross-site routing.
    #
    # The tier-1 lane uses study_type_block() with no site-specific keyword
    # filter, so it retrieves ALL cancer trial papers from NEJM/Lancet/JCO/JAMA
    # regardless of disease site. That's intentional for recall — a thyroid
    # cancer trial in Lancet Oncol should be caught by head_neck even if
    # "thyroid" isn't in head_neck's core_block.
    #
    # But without routing, whichever site runs first (alphabetically: breast)
    # claims every tier-1 PMID via cross-site dedup, then tags prostate/NSCLC/
    # RCC papers as "breast". The fix mirrors the FDA routing logic: for each
    # tier-1 record that does NOT match the current site's quick terms, skip it
    # if it clearly belongs to another site — that site's run will claim it.
    # Records matching no site's quick terms (ambiguous) are kept.
    tier1_pmid_set = set(tier1_pmids)
    _cur_site_terms = [t.lower() for t in SITE_QUICK_TERMS.get(site.code, [])]
    routed: list = []
    n_rerouted = 0
    for r in fetched:
        if r.pmid in tier1_pmid_set:
            _text = f"{r.title} {r.abstract}".lower()
            _matches_current = not _cur_site_terms or any(t in _text for t in _cur_site_terms)
            if not _matches_current:
                _other = next(
                    (code for code, terms in SITE_QUICK_TERMS.items()
                     if code != site.code and any(t.lower() in _text for t in terms)),
                    None,
                )
                if _other:
                    log.info(
                        "[%s] tier-1 routing: PMID %s belongs to %s — skipping (title: %s)",
                        site.code, r.pmid, _other, r.title[:80],
                    )
                    n_rerouted += 1
                    continue
        routed.append(r)
    if n_rerouted:
        log.info("[%s] tier-1 routing: dropped %d records routed to other sites", site.code, n_rerouted)
    fetched = routed
