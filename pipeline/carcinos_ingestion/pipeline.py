...
from .disease_sites.base import DiseaseSiteConfig, TIER1_JOURNAL_WHITELIST
...
# Max concurrent LLM calls per site. Keeps us well within OpenAI rate limits
# while still getting strong parallelism gains.
_LLM_WORKERS = 8
...
def run_site(
    ...
    *,
    ...
    # ---- Tier-1 bypass: NEJM/Lancet/JCO/JAMA skip Pass 1 entirely ------
    # Papers from top-tier journals are pre-screened by editorial standards;
    # sending them straight to Pass 2 saves one LLM call per paper.
    tier1_whitelist = [j.lower() for j in TIER1_JOURNAL_WHITELIST]
    pass1_kept: list[tuple[CanonicalCandidate, Pass1Result]] = []
    tier1_bypass_count = 0

    def _is_tier1(cand: CanonicalCandidate) -> bool:
        j = (cand.journal or "").lower()
        return any(w in j or j in w for w in tier1_whitelist)

    # Separate tier-1 (bypass Pass 1) from others (need Pass 1)
    tier1_cands = [c for c in canonicals if _is_tier1(c)]
    other_cands  = [c for c in canonicals if not _is_tier1(c)]
    tier1_bypass_count = len(tier1_cands)
    if tier1_bypass_count:
        log.info("[%s] %d tier-1 papers bypass Pass 1 (NEJM/Lancet/JCO/JAMA)",
                 site.code, tier1_bypass_count)
        # Synthesise a minimal Pass1Result so Pass 2 has context
        from .triage.pass1 import Pass1Result as _P1R
        for c in tier1_cands:
            dummy_p1 = _P1R(
                parsed={"keep": True, "relevance_score": 90, "priority_hint": "high",
                        "one_line_rationale": "Tier-1 journal bypass"},
                cost_usd_estimate=0.0,
            )
            pass1_kept.append((c, dummy_p1))

    # ---- STEP 7  Pass 1 — parallel for non-tier-1 candidates ------------
    _stats_lock = threading.Lock()

    def _do_pass1(cand: CanonicalCandidate):
        p1 = run_pass1(openai_client, cand)
        keep, _ = enforce_pass1_keep_rules(
            cand, p1,
            keep_threshold=config.keep_threshold,
    ...
    with ThreadPoolExecutor(max_workers=_LLM_WORKERS) as pool:
        futures = {pool.submit(_do_pass1, c): c for c in other_cands}
        for fut in as_completed(futures):
            try:
                cand, p1, keep = fut.result()
                with _stats_lock:
                ...
                log.exception("[%s] pass1 failure pmid=%s", site.code, cand.pmid)

    stats.pass1_kept_count = len(pass1_kept)
    log.info("[%s] %d after Pass 1 keep rules (%d tier-1 bypassed)",
             site.code, stats.pass1_kept_count, tier1_bypass_count)

    # ---- STEP 8  Pass 2 — parallel deep review ---------------------------
    alerts: list[AlertOutput] = []
    alerts_lock = threading.Lock()

    def _do_pass2(cand: CanonicalCandidate, p1: Pass1Result):
        p2 = run_pass2(openai_client, cand, p1.parsed)
        grounded, unverified = verify_evidence_quotes(p2.parsed, cand.abstract)
        return cand, p1, p2, grounded, unverified

    with ThreadPoolExecutor(max_workers=_LLM_WORKERS) as pool:
        futures2 = {pool.submit(_do_pass2, c, p): (c, p) for c, p in pass1_kept}
...
def _retrieve(
    ...
    *,
    ...
    # Lane 1 (main broad query) DISABLED — searching all of PubMed by keyword
    # pulls too many lower-tier journals. Practice-changing results appear in
    # major journals; whitelist lanes below are sufficient and much cleaner.
    main_pmids: list[str] = []

    # Lane 2a: tier-1 guaranteed lane (NEJM/Lancet/JCO/JAMA) — never capped
    # so high-impact papers are never crowded out by lower-tier journals.
    ...
    # Lane 2b: broader journal force-include (rest of whitelist)
    ...
    if include_conferences:
        ...
        conf_records = retrieve_conference_records(
            ...
            reference_date=end,
            lookback_days=35,
            max_pmids=max_pmids // 3,  # ~25 PMIDs per meeting — enough for top abstracts
        )
        log.info("[%s] conference lane: %d records", site.code, len(conf_records))

    # Merge: dedupe by PMID at the ID level before efetch
    all_pmids = list({p for p in (main_pmids + tier1_pmids + journal_pmids) if p})
