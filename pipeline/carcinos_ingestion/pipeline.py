...
class RunStats:
    ...
    # ── LLM-first attrition instrumentation ──────────────────────────────────
    # Under LLM-first, QS_NONE papers are no longer dropped at the gate. These
    # counters let an operator measure exactly what the gate used to discard and
    # how much of it the LLM recovers — the measurement the assessment asked for.
    ...
    qs_none_sent_to_llm: int = 0                 # QS_NONE forwarded to Pass 2 after cap
    qs_none_recovered_count: int = 0             # QS_NONE that the LLM tiered A/B/C
...
def run_site(
    ...
    *,
    ...
    # Rank + cap the no-signal tier: top-tier journal first, then deterministic
    # relevance. The cap bounds cost — the qualifying tier is never capped here.
    if no_signal:
        no_signal.sort(
            key=lambda x: (
        ...
        if qs_none_cap and qs_none_cap > 0:
            no_signal = no_signal[:qs_none_cap]
        stats.qs_none_sent_to_llm = len(no_signal)

    log.info(
        "[%s] Pass 2 intake: %d qualifying + %d no-signal (cap=%s) | "
        "hard-excluded=%d qs_none_total=%d (top-journal qs_none=%d)",
        site.code,
        stats.signal_kept_count,
        stats.qs_none_sent_to_llm,
        qs_none_cap,
    ...
    # the broad (no-signal) tier runs on the cheaper triage model to bound cost.
    work: list[tuple[CanonicalCandidate, SignalScore, str, bool]] = [
        (c, s, config.deep_review_model, False) for c, s in signal_kept
    ] + [
        (c, s, config.triage_model, True) for c, s in no_signal
    ]
