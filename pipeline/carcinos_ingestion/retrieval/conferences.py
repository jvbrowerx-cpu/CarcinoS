...
def build_conference_query(
    ...
    date_block = f'("{date_str_start}"[Date - Publication] : "{date_str_end}"[Date - Publication])'

    journal_block = f'"{meeting.journal}"[Journal]'
    pubtype_block = '"congress"[Publication Type]'
    core_block = site.core_block()

    # Narrow to trial results only — excludes educational talks, state-of-the-art
    # reviews, and case reports that dominate meeting supplement volumes.
    trial_filter = (
        '("randomized" OR "phase 2" OR "phase II" OR "phase 3" OR "phase III" OR '
        '"overall survival" OR "progression-free" OR "FDA" OR "approval" OR "guideline")'
    )

    return f"{core_block} AND {journal_block} AND {date_block} AND {pubtype_block} AND {trial_filter}"
...
def retrieve_conference_records(
    ...
    records = pubmed.efetch(list(all_pmids))
