...
def _fetch_fda_via_openfda(cutoff: date, timeout: int = 20) -> list[FDAItem]:
    ...
    for result in results:
        ...
        # Oncology filter: pharmacological class OR drug name against _ONCO_RE
        check_text = " ".join(pharm_classes) + " " + drug_name + " " + sub_class
        if not _ONCO_RE.search(check_text) and not _ONCO_PHARM_CLASS_RE.search(
            " ".join(pharm_classes)
        ):
            continue
...
def fetch_fda_oncology_items(
    ...
    # ── Source 3: LLM web search supplement (runs when API key available) ────
    ...
    if openai_api_key:
        if not items:
            ...
            log.info(
                "FDA LLM supplement: cross-checking %d item(s) for missed approvals",
                len(items),
            )
