...
def fetch_fda_oncology_items(
    ...
    # LLM cross-check — runs ALWAYS when a key is available, not just when
    # HTML/RSS return zero. This is intentional: if the HTML scrape returns
    # some approvals but misses one (e.g. EV-304 was posted after the scrape,
    # a table row was partially rendered, or Akamai returned a cached page),
    # the old "if not items" guard would silently suppress the LLM supplement.
    # We now merge LLM results with HTML/RSS results by URL dedup, so there
    # are no duplicates and no coverage gaps.
    if openai_api_key:
        if not items:
            log.warning(
                "FDA HTML+RSS returned 0 items — running LLM as primary fallback"
            )
        else:
            log.info(
                "FDA LLM supplement: cross-checking %d HTML/RSS item(s) for missed approvals",
                len(items),
            )
        ...
        if llm_items:
            ...
            if new_items:
                log.info(
                    "FDA LLM supplement: %d new item(s) not found by HTML/RSS: %s",
                    len(new_items),
