...
def _scrape_approval_page(
    ...
    for tr in tr_blocks:
        ...
        # IMPORTANT: also skip rows where date couldn't be parsed (pub_date_obj
        # is None) — the old `if pub_date_obj and ...` silently let undated rows
        # through, causing years-old approvals to bypass the cutoff filter.
        if not pub_date_obj or pub_date_obj < cutoff:
            continue
...
def _item_to_pubmed_record(item: FDAItem) -> PubMedRecord:
    ...
    return PubMedRecord(
        ...
        publication_types=["FDA Drug Approval"],
        doi=item.url,   # store FDA URL here so it surfaces as the "Read paper" link
        pmc_id=None,
        nct_ids=[],
        mesh_terms=[],
