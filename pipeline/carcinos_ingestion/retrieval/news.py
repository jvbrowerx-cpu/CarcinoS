...
def fetch_society_news_via_llm(
    ...
    raw = _run_llm_web_search(openai_client, prompt, model, "society-news")
    if not raw:
        return []
    ...
    for m in _SOCIETY_ITEM_RE.finditer(raw):
        ...
        records.append(PubMedRecord(
            ...
            raw_xml=raw_text,
            text_hash=text_hash,
            conference_source="SOCIETY_NEWS",
        ))
...
def fetch_secondary_via_llm(
    ...
    raw = _run_llm_web_search(openai_client, prompt, model, "secondary-trial-names")
    if not raw:
        return []
...
def run_news_lane(
    ...
    Args:
        ...
        openai_client:    OpenAI client for LLM web search. If None, LLM
    ...
    if openai_client is None:
        log.warning(
            "[Lane6] No openai_client provided — ASCO/ESMO/AACR society news "
            "and trade press trial-name discovery are disabled. "
            "Pass openai_client to run_news_lane() to enable LLM web search."
        )
    else:
