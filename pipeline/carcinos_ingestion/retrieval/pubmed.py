...
# ---------------------------------------------------------------------------
# PMC full-text retrieval (living guideline enrichment path)
# ---------------------------------------------------------------------------
# Uses the standard NCBI E-utilities efetch endpoint (db=pmc) — the stable
# Entrez route, distinct from the PMC distribution services (pmc/utils/oa/)
# which are undergoing retirement/reorganisation.

_RECOMMENDATION_KEYWORDS = frozenset([
    "updated evidence and recommendation",
...
def fetch_pmc_fulltext(
    ...
    email: str = "carcinos@pipeline",
    timeout: int = 30,
) -> str:
    """
    Fetch full-text XML from PMC for an open-access article and return
    the extracted recommendation-relevant section text (up to 8 000 chars).

    Returns empty string if the article is not in PMC OA, the endpoint
    is unavailable, or no parseable content is returned.  Callers should
    treat an empty return as a signal to fall back to web-search enrichment.
    """
    if not pmc_id:
        return ""
    pmc_numeric = pmc_id.upper().replace("PMC", "").strip()
    if not pmc_numeric:
        return ""
    params: dict = {
        "db": "pmc",
    ...
    try:
        ...
        if r.status_code != 200:
            return ""
        return _extract_guideline_sections(r.text)
    except Exception:
        return ""


def _extract_guideline_sections(xml_text: str) -> str:
    """
    Parse PMC JATS XML and extract sections relevant to guideline updates.

    Prioritises sections whose titles contain recommendation-change language
    (e.g. "Updated Evidence and Recommendations", "New Recommendations").
    Returns plain text — priority sections first, then others — capped at
    8 000 chars so it fits the LLM context alongside the rest of the prompt.
    """
    try:
    ...
    # JATS XML uses a namespace; handle both namespaced and bare tags.
    _SEC_TAGS = {"sec", "{https://jats.nlm.nih.gov}sec", "{http://jats.nlm.nih.gov}sec"}
    _TITLE_TAGS = {"title", "{https://jats.nlm.nih.gov}title", "{http://jats.nlm.nih.gov}title"}
    _P_TAGS = {"p", "{https://jats.nlm.nih.gov}p", "{http://jats.nlm.nih.gov}p"}

    for sec in root.iter():
        if sec.tag not in _SEC_TAGS:
            continue

        title_el = next((c for c in sec if c.tag in _TITLE_TAGS), None)
        ...
        paragraphs = [
            ...
            for c in sec
            if c.tag in _P_TAGS and "".join(c.itertext()).strip()
        ]
        if not paragraphs:
            continue

        block = (f"[{title_text}]\n" if title_text else "") + "\n".join(paragraphs[:8])
        if any(kw in title_lower for kw in _RECOMMENDATION_KEYWORDS):
            priority.append(block)
        else:
            other.append(block)

    return "\n\n".join(priority + other)[:8000]
