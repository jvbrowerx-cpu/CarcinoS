...
`web_search_preview` built-in tool, giving the LLM browsing capability to surface:
...
def run_web_search_lane(
    ...
    *,
    ...
    try:
        ...
        # We do NOT use strict json_schema format here — combining strict schema
        # enforcement with web_search_preview tool-use events causes conflicts
        # because the model interleaves tool calls with its text response.
        # Instead we use json_object format + parse with a robust extractor.
