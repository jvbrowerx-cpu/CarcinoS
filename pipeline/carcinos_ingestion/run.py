...
def main(argv: list[str] | None = None) -> int:
    ...
    # Run all sites in parallel (max 5 concurrent to respect OpenAI rate limits).
    # Supabase deduplicates via text_hash unique constraint so cross-site
    # duplicate handling is handled at the DB layer — no shared seen_pmids needed.
    _SITE_WORKERS = 5 if len(sites) > 1 else 1
...
def _print_card(a: AlertOutput) -> None:
    ...
    print(f"{tb} {label} {_RESET}{conf_badge}  "
          f"{_DIM}Evidence {a.pass2.evidence_strength} · Impact {a.pass2.impact_score}{_RESET}")
