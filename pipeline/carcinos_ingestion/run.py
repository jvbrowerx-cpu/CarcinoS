...
def main(argv: list[str] | None = None) -> int:
    ...
    attrition = {
        ...
        "qs_none_sent_to_llm":      _sum("qs_none_sent_to_llm"),
    ...
    for k in ("raw_retrieved", "after_dedupe", "after_pubtype", "after_relevance",
              "qualifying_signal_kept", "qs_none_total", "qs_none_sent_to_llm",
              "qs_none_recovered", "pass2_runs"):
        logging.info("  %-26s %d", k, attrition[k])
    if qs_none_recovered:
        logging.info(
            "  → %d papers recovered that the gate would have dropped:",
