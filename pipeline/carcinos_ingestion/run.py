...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for row in (_existing.data or []):
            ...
            # Extract title for sentinel (used when PMID is absent)
            row_title = (sj.get("trial") or {}).get("name") or ""
            row_has_pmid = False
            for src in sj.get("sources", []):
                ...
                if pmid:
                    ...
                    row_has_pmid = True
            # No PMID on any source — add title sentinel so PMID-less records
            # (e.g. ASCO living guidelines fetched without a PubMed ID) are
            # still blocked on re-runs.
            if not row_has_pmid and row_title:
                sentinel = "title:" + " ".join(row_title.lower().split())[:200]
    ...
    # Pre-load FDA-approved trial names (1-year lookback) for cross-event dedup.
    # When CarcinoS previously captured the FDA approval for a trial, the primary
    # manuscript publication of that trial = same clinical event and should not
    # generate a second card. We store these names and suppress matching manuscripts
    # after Pass 2 (where trial_name is extracted by the LLM).
    fda_approved_trial_names: set[str] = set()
    try:
        from datetime import timedelta as _td_fda
        _fda_cutoff = (end - _td_fda(days=365)).isoformat()
        _fda_rows = (
            _sb_dedup.table("alerts")
            .select("summary_json")
            .gte("created_at", _fda_cutoff)
            .execute()
        )
        for row in (_fda_rows.data or []):
            sj = row.get("summary_json") or {}
            if (sj.get("qualifying_signal") or "").upper() == "FDA":
                tn = (sj.get("trial_name") or "").strip().lower()
                if tn:
                    fda_approved_trial_names.add(tn)
        logging.info(
            "[fda_dedup] pre-loaded %d FDA trial names from Supabase (1-year window)",
            len(fda_approved_trial_names),
        )
    except Exception as _fda_err:
        logging.warning("[fda_dedup] could not pre-load FDA trial names: %s", _fda_err)
    ...
    # ── FDA cross-event deduplication ────────────────────────────────────────
    # A primary manuscript for a trial whose FDA approval CarcinoS already
    # captured is the same clinical event — not new information for clinicians.
    # Drop non-FDA alerts whose trial_name matches a previously-captured FDA
    # approval (pre-loaded from Supabase above). FDA alerts this week are kept.
    if fda_approved_trial_names:
        fda_dedup_kept: list[AlertOutput] = []
        fda_dedup_dropped: list[AlertOutput] = []
        for a in deduplicated:
            tn = (a.pass2.parsed.get("trial_name") or "").strip().lower()
            qs = (a.pass2.parsed.get("qualifying_signal") or "").upper()
            # Never drop the FDA alert itself, or alerts with no trial name.
            if not tn or qs == "FDA":
                fda_dedup_kept.append(a)
            elif tn in fda_approved_trial_names:
                fda_dedup_dropped.append(a)
                logging.info(
                    "[fda_dedup] CROSS-EVENT DEDUP: trial='%s' pmid=%s — FDA approval "
                    "already captured; suppressing this week's manuscript",
                    tn, a.candidate.pmid or "?",
                )
            else:
                fda_dedup_kept.append(a)
        if fda_dedup_dropped:
            logging.info(
                "[fda_dedup] suppressed %d manuscript(s) with prior FDA approval",
                len(fda_dedup_dropped),
            )
        deduplicated = fda_dedup_kept
        trial_dropped.extend(fda_dedup_dropped)
    ...
    # ── Auto-backup Supabase tables ──────────────────────────────────────────
    if persist:
        try:
            from .backup import run_backup
            run_backup(cfg.supabase_url, cfg.supabase_service_role_key)
        except Exception as _bk_err:
            logging.warning("[backup] Supabase backup failed (non-fatal): %s", _bk_err)
