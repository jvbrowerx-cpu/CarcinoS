...
Output rules:
  ...
  • Do NOT duplicate entries that are already in the "already_watching" list.
...
Already watching (exclude from your output — they're covered):
{existing_trials}

What specific trial acronyms or NCT numbers should we search for in PubMed \
over the past 21 days for this disease site? Return JSON only."""
...
def generate_context_brief(
    ...
    *,
    ...
    # Deduplicate against existing watchlist (case-insensitive)
    existing_lower = {e.lower() for e in existing_trials}
    new_trials = [t for t in cleaned if t.lower() not in existing_lower]
