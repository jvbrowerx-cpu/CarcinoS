...
def enforce_pass1_keep_rules(
    ...
    # EXCEPTION: if the LLM explicitly says wrong_disease_site, respect it
    # even for top-tier journals. A NEJM AML paper retrieved by the breast
    # pipeline should still be dropped — force-keep only bypasses signal
    # scoring gates, not disease-site confirmation.
    if candidate.force_keep_reason:
        if pass1.drop_reason == "wrong_disease_site" and not pass1.keep:
            return False, "wrong_disease_site (overrides force_keep)"
