...
def enforce_pass1_keep_rules(
    ...
    """
    # Hard drop for wrong disease site — applies unconditionally before any
    # score or force-keep override. A salivary gland paper retrieved by the
    # breast lane should never reach Pass 2 regardless of its relevance score
    # or journal prestige. Previously this check only lived inside the
    # force_keep block, so high-scoring wrong-site papers slipped through.
    if pass1.drop_reason == "wrong_disease_site" and not pass1.keep:
        return False, "wrong_disease_site"
