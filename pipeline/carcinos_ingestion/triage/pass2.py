...
def run_pass2_guideline(
    ...
    Architecture (living-guideline enrichment path):
    ─────────────────────────────────────────────────
    Living guidelines (ASCO, ESMO, etc.) publish generic JCO abstracts that
    do not contain the actual recommendation delta.  For these we attempt
    PMC full-text enrichment BEFORE the first LLM call, so the model has
    the "Updated Evidence and Recommendations" section rather than the
    boilerplate abstract.

    Enrichment order:
      1. If living guideline detected AND pmc_id known → fetch PMC full text
         via NCBI eutils efetch (db=pmc) — the stable Entrez route.
      2. Use full text in place of abstract for the primary structured call.
      3. If full text unavailable or still no concrete delta → web search.
      4. If all enrichment fails → DELTA_NOT_FOUND logged; card dropped as
         NOISE with a clear audit trail for editorial follow-up.
    """
    ...
    # ── Step 1: Detect living guideline and attempt upfront PMC enrichment ────
    is_living_guideline = "living guideline" in (candidate.title or "").lower()
    pmc_fulltext = ""
    fulltext_source: str | None = None

    if is_living_guideline and getattr(candidate, "pmc_id", None):
        from ..retrieval.pubmed import fetch_pmc_fulltext
        _log.info(
            "guideline pass2: living guideline detected — fetching PMC full text "
            "(pmc_id=%s, title: %.60s)",
            candidate.pmc_id, candidate.title,
        )
        pmc_fulltext = fetch_pmc_fulltext(candidate.pmc_id)
        if pmc_fulltext:
            fulltext_source = "PMC_FULLTEXT"
            _log.info(
                "guideline pass2: PMC full text retrieved (%d chars) for '%.60s'",
                len(pmc_fulltext), candidate.title,
            )
        else:
            _log.info(
                "guideline pass2: PMC full text not available (pmc_id=%s) — "
                "will fall back to web search if abstract extraction fails",
                candidate.pmc_id,
            )

    # ── Step 2: Build user prompt — full text preferred over abstract ─────────
    source_label = "FULL TEXT (PMC)" if pmc_fulltext else "ABSTRACT"
    content_for_llm = (
        f"[SOURCE: {source_label}]\n{pmc_fulltext[:8000]}"
        if pmc_fulltext
        else (candidate.abstract or "")[:8000] or "(no abstract available)"
    )

    user = PASS2_GUIDELINE_USER_TEMPLATE.format(
        title=candidate.title or "(no title)",
        abstract=content_for_llm,
        journal=candidate.journal or "(unknown)",
        publication_date=candidate.publication_date or "(unknown)",
        publication_types=", ".join(candidate.publication_types) or "(none)",
        mesh_terms=", ".join(candidate.mesh_terms[:20]) or "(none)",
    ...
    if fulltext_source:
        parsed["why_it_surfaced"] = (
            ...
            + f" [content from {fulltext_source}]"
        ).strip()

    evidence = str(parsed.get("evidence_strength", "D"))

    # ── Step 3: Quality gate — fall back to web search if still no delta ─────
    _BLANK_VALUES = {None, "", "not_reported", "Not applicable",
                     "N/A", "n/a", "not applicable", "None"}

    def _has_concrete_changes(p: dict) -> bool:
        return any(
            c.get("new") not in _BLANK_VALUES
            for c in (p.get("recommendation_changes") or [])
        )

    if not parsed.get("reject_recommendation", False) and not _has_concrete_changes(parsed):
        _log.info(
            "guideline pass2: %s extraction yielded no concrete changes — "
            "attempting web search enrichment (title: %.80s)",
        ...
        if web_changes:
            ...
            parsed["why_it_surfaced"] = (
                (parsed.get("why_it_surfaced") or "")
                + " [changes sourced via web search — primary text was non-specific]"
            ).strip()
            _log.info(
                "guideline pass2: web enrichment found %d concrete change(s) for '%.60s'",
                len(web_changes), candidate.title,
            )
        else:
            # All enrichment paths exhausted — flag clearly and drop as NOISE.
            # The DELTA_NOT_FOUND marker in why_it_surfaced is the audit hook
            # for a future VERIFY routing stage.
            parsed["reject_recommendation"] = True
            evidence = "D"
            parsed["why_it_surfaced"] = (
                (parsed.get("why_it_surfaced") or "")
                + " [DELTA_NOT_FOUND: abstract"
                + (", PMC full text" if is_living_guideline else "")
                + ", and web search all failed to extract a concrete recommendation "
                "change — requires editorial verification]"
            ).strip()
            _log.warning(
                "guideline pass2: all enrichment paths failed for '%.80s' "
                "(pmc_id=%s) — DELTA_NOT_FOUND, dropping as NOISE",
                candidate.title, getattr(candidate, "pmc_id", "none"),
            )

    # ── Step 4: Tier assignment ───────────────────────────────────────────────
    # Only evidence_strength A (concrete practice-changing recommendation)
    # survives — B/C guideline updates are dropped since the underlying trial
    # is almost always captured separately by the PubMed lane.
    ...
    rationale_text = (
        f"Tier {final_code} — GUIDELINE extraction ({source_label}): "
        f"{body} {version}, {disease}. "
