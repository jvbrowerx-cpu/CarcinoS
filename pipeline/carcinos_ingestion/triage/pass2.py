...
─────────────────────────────────────────────────────────────────────────────
CLINICIAN RELEVANCE TEST — apply before reject_recommendation
─────────────────────────────────────────────────────────────────────────────
Ask yourself: would a tumor board physician reasonably say "Do I need to know
this for clinic, tumor board, patient counseling, or upcoming guideline changes?"

  KEEP if yes — a tumor board physician would need this.
  REJECT if the honest answer is only: "This is academically interesting."

This is the most important filter. Journal prestige is not a reason to keep
something. A Phase III trial in a prestigious journal that is clinically
uninterpretable should be rejected. A Phase II result in a minor journal that
directly changes a treatment decision should be kept.
...
Set reject_recommendation=true if the paper is noise despite passing the gate:
  ...
  • Fails the clinician relevance test above — academically interesting only
  ...
  • Meta-analysis or network meta-analysis that simply reconfirms an already
    FDA-approved regimen without new comparators or populations — adds no
    new clinical decision
  • Consensus review / expert opinion that provides no new primary data and
    only summarises existing evidence without a formal guideline revision
  ...
  • Phase III trial with clinically uninterpretable result — missing HR, p-value,
    or primary endpoint makes the result impossible to act on
...
2. Apply the clinician relevance test: would a tumor board physician need to
   know this for clinic, tumor board, counseling, or guideline changes?
   If the honest answer is only "academically interesting," set
   reject_recommendation=true.

3. Use why_it_surfaced to record the strongest clinical justification.
   If rejecting, explain in why_it_surfaced why it is noise.
...
def run_pass2_guideline(
    ...
    # evidence_strength C/D, or reject_recommendation=true → NOISE
