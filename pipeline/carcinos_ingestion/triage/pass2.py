...
REJECT (reject_recommendation=true) only for these specific categories:
  ...
  • Non-oncology population — primary study population consists of non-cancer patients
    (e.g., chronic musculoskeletal pain, cardiovascular disease, diabetes, general chronic pain)
    with no cancer-specific subgroup or oncology indication. If the study population contains
    no patients with cancer diagnoses, reject regardless of journal or study design.
...
def run_pass2_guideline(
    ...
    if not parsed.get("reject_recommendation", False):
        ...
        if not has_concrete_change:
            ...
            _log.getLogger("carcinos.pipeline").debug(
                "guideline pass2: no extractable recommendation changes — "
                "overriding reject_recommendation=True (title: %s)",
                candidate.title[:80],
            )
