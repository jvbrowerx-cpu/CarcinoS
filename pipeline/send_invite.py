...
# ── Config ────────────────────────────────────────────────────────────────────
SIGNUP_URL   = "https://carcino-s.com/?signup=true"
...
<title>CarcinoS — New member portal</title>
</head>
    ...
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      ...
      <tr><td style="background:#0e0e14;border:1px solid rgba(255,255,255,0.10);border-radius:14px;padding:36px 36px 32px;">
        ...
        <h1 style="margin:0 0 16px;font-size:24px;font-weight:800;color:#ffffff;letter-spacing:-0.5px;line-height:1.2;">
          We've launched a member portal
        </h1>
        <p style="margin:0 0 20px;font-size:15px;color:#b0b0bc;line-height:1.6;">
          CarcinoS has been updated with a member portal where you can read this week's digest online,
          browse the full article archive, and discuss findings directly on articles with other oncologists.
        </p>
        ...
        <table cellpadding="0" cellspacing="0" style="margin-bottom:24px;width:100%;">
          <tr>
            <td style="padding:12px 16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:10px;">
              <p style="margin:0 0 10px;font-size:13px;color:#b0b0bc;line-height:1.7;">
                <span style="color:#72a37a;font-weight:700;">→</span>&nbsp; Read this week's digest online, organized by disease site
              </p>
              <p style="margin:0 0 10px;font-size:13px;color:#b0b0bc;line-height:1.7;">
                <span style="color:#72a37a;font-weight:700;">→</span>&nbsp; Browse the full article archive
              </p>
              <p style="margin:0;font-size:13px;color:#b0b0bc;line-height:1.7;">
                <span style="color:#72a37a;font-weight:700;">→</span>&nbsp; Comment on articles and discuss findings with colleagues
              </p>
            </td>
          </tr>
        </table>
        ...
        <p style="margin:0 0 24px;font-size:14px;color:#b0b0bc;line-height:1.6;">
          Create your free account using <strong style="color:#ffffff;">{email}</strong> and choose a password.
          You'll also be asked to set up a short profile (name, credentials, institution)
          so your comments are attributed correctly.
        </p>
        ...
        <p style="margin:0;font-size:12px;color:rgba(255,255,255,0.35);line-height:1.6;">
          Your weekly email digest will continue as normal regardless.
          The portal is an optional addition for those who want online access and discussion.
        </p>
...
def main() -> None:
    ...
    all_subscribers = resp.data or []

    if not all_subscribers:
        print("No verified subscribers found.")
        return

    # Fetch all auth emails so we only invite those without an account
    auth_resp = sb.auth.admin.list_users()
    auth_emails = {u.email.lower() for u in auth_resp if u.email}

    subscribers = [u for u in all_subscribers if u.get("email", "").lower() not in auth_emails]

    print(f"Found {len(all_subscribers)} subscriber(s), {len(subscribers)} without auth accounts.")
    if not subscribers:
        print("All subscribers already have accounts — nothing to send.")
        return

    print(f"{'[DRY RUN] ' if dry_run else ''}Sending invites to {len(subscribers)}...\n")
